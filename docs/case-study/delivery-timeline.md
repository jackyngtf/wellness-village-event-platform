...
I wanted the client to react to a working page before every production detail was ready. I therefore built a small first version on my home server, then spent the following two weeks working through the content, integrations, UX feedback, privacy checks and production setup.
...
| **21–29 August** | I continued with search setup, copy and image corrections, venue-map details, contact handling and launch checks. | Sharing the URL was a release milestone, not the end of the work. |
