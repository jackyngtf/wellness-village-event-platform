...
The first Agent brief included:
...
For The Ground, the adapter limits pagination, waits no longer than eight seconds, rejects responses above 2 MiB, validates the response at runtime, converts times to HKT and keeps a five-minute in-memory copy. See [The Ground integration](../case-study/04-the-ground-event-interface.md).
...
It leaves out credentials, production identifiers, personal data, private messages, raw Agent transcripts and source material that cannot be redistributed. The [code-and-test index](../case-study/10-evidence-index.md) and [release checklist](release-checklist.md) help keep that separation clear.
